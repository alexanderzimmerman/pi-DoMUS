...
private:
  ...
  /**
   * Temperature field for bouyancy term
   **/
  FEFieldFunction<2> temperature_field;
                       ...
                       bool compute_only_system_terms) const
{
  ...
  */
  auto &pressure_points = fe_cache.get_quadrature_points("solution", "p", pressure, et);
      ...
      for (unsigned int i=0; i<residual[0].size(); ++i)
        {
		  ...
		  // Bouyancy term (for Navier-Stokes-Boussinesq)
		  double g = -10., beta = 1., T0 = 0.;
		  double T;
